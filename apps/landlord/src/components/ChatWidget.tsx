...
export function ChatPanel({ onClose, embedded = false, initialInput }: { onClose?: () => void; embedded?: boolean; initialInput?: string }) {
  ...
  async function send() {
    ...
    // was. Nic: show that "he's helping another customer right now". The gate
    // has held conversations back since S628 and the person waiting saw a
    // typing indicator running for ninety seconds with no explanation, which
    // reads as a broken page rather than as a busy person.
    //
    // Starts after a beat: a turn that gets a slot immediately, which is nearly
    // all of them, must never flash a "one moment" that was never true.
    let polling = true
    void (async () => {
...
function Waiting({ agent, note }: { agent: string; note: string }) {
  return (
    <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
      ...
      <div style={{ padding: '9px 12px', borderRadius: 12, background: 'var(--bg-3)', color: 'var(--text-2)', fontSize: 13, fontStyle: 'italic' }}>
        {note}
      </div>
    </div>
  )
