...
/**
 * In-app camera capture (B3). Opens the device camera via getUserMedia and
 * captures a PHOTO (canvas frame → JPEG) or a VIDEO (MediaRecorder → WebM)
 * directly — there is NO file-picker path, so what's captured is fresh from
 * the lens, not pulled from the gallery. Uploads tagged capturedLive=true by
 * the caller. Not tamper-proof on the web (a virtual camera can still spoof);
 * the strong guarantee waits for the native app.
 */
export function CameraCapture({
  ...
  const cancelRef = useRef(false) // true → a stop triggered by unmount/cancel, don't save
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (recorderRef.current?.state === 'recording') {
        cancelRef.current = true // closing mid-record cancels — don't save a partial
  ...
  function startRecording() {
    ...
    rec.onstop = () => {
      if (cancelRef.current) return // unmount/cancel — discard
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.85)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', zIndex: 1100 }}>
      <button className="btn btn-ghost btn-sm" onClick={onClose} style={{ position: 'absolute', top: 16, right: 16, color: '#fff' }}>
        <X size={18} /> Close
      </button>
