...
export function SchedulePage() {
  ...
  const getBookingForDate = (unitId: string, date: string) => {
    return bookings.find(b => b.unitId === unitId && date >= b.checkIn && date < b.checkOut) ||
           leases.find(l => l.unitId === unitId && date >= l.startDate && date <= l.endDate)
  }
  ...
  const onDrop = (e: React.DragEvent, targetUnitId: string, targetDate: string) => {
    ...
    if (targetUnit.leaseTypesAllowed && !targetUnit.leaseTypesAllowed.includes(b.leaseType)) {
      alert(`That unit does not allow ${b.leaseType} bookings.`)
      dragBooking.current = null
      dragOverRef.current = null
      return
    }
    ...
    const hasBookingConflict = bookings.some(existing =>
      ...
      rangeDays.some(d => d >= existing.checkIn && d < existing.checkOut)
    )
    const hasLeaseConflict = leases.some(l =>
      l.unitId === targetUnitId &&
      rangeDays.some(d => d >= l.startDate && d <= l.endDate)
    )
              ...
              {filteredUnits.map(unit => (
                <tr key={unit.id} style={{height:72,maxHeight:72}}>
                  ...
                  {days.map(d => {
                    ...
                    const isStart = booking && (booking.checkIn === d || booking.startDate === d)
        ...
        <div className="modal-overlay" onClick={()=>setUnitPickerOpen(false)}>
          <div className="modal" style={{maxWidth:420}} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{padding:'0 24px 24px',display:'grid',gap:8}}>
              {units.filter(u=>u.isBookable).length === 0 && (
              ...
              {units.filter(u=>u.isBookable).map(u=>(
                <button key={u.id} className="btn btn-ghost" style={{textAlign:'left',display:'flex',alignItems:'center',justifyContent:'space-between',gap:12}}
                  ...
                  <div>
                    <div style={{fontWeight:600,fontSize:'.85rem'}}>{u.unitNumber}</div>
                    <div style={{fontSize:'.72rem',color:TYPE_COLORS[u.unitType]||'var(--text-3)'}}>{UNIT_TYPE_LABELS[u.unitType]||u.unitType} · {u.propertyName}</div>
                  </div>
                  <div style={{textAlign:'right',fontSize:'.75rem',color:'var(--gold)',fontWeight:600}}>
                    {u.nightlyRate ? fmt(u.nightlyRate)+'/night' : u.weeklyRate ? fmt(u.weeklyRate)+'/wk' : ''}
                  </div>
                </button>
              ))}
            </div>
