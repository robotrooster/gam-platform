...
export function ShelfLabelPage() {
  ...
  const priceWithTax = item.sell_price * (1 + item.tax_rate)
      ...
      <div style={{ width: '100%', maxWidth: 400, textAlign: 'center' }}>
        ...
        <div style={{ background: 'linear-gradient(135deg, #1a1600, #2a2200)', border: '2px solid rgba(201,162,39,.4)', borderRadius: 20, padding: '28px 32px', marginBottom: 20 }}>
          ...
          {item.tax_rate > 0 && (
            <div style={{ fontSize: '.75rem', color: '#7a8aaa', marginTop: 8 }}>
              +{(item.tax_rate * 100).toFixed(0)}% tax · {formatCurrency(priceWithTax)} after tax
            </div>
          )}
        </div>
        ...
        <div style={{ background: '#0a0d10', border: '1px solid #1e2530', borderRadius: 12, padding: '14px 20px', marginBottom: 16 }}>
          ...
          <div style={{ fontFamily: 'system-ui', fontSize: '1.5rem', fontWeight: 800, color: '#4a9eff' }}>
            {formatCurrency(item.sell_price * 1.03)}
          </div>
        </div>

        {/* Stock indicator */}
        {item.stock_qty < 999 && (
          <div style={{ fontSize: '.72rem', color: item.stock_qty <= 5 ? '#ff4757' : item.stock_qty <= 10 ? '#ffb820' : '#1edb7a', padding: '6px 14px', borderRadius: 20, background: item.stock_qty <= 5 ? 'rgba(255,71,87,.08)' : item.stock_qty <= 10 ? 'rgba(255,184,32,.08)' : 'rgba(30,219,122,.08)', display: 'inline-block', marginBottom: 16 }}>
            {item.stock_qty <= 5 ? `⚠️ Only ${item.stock_qty} left!` : item.stock_qty <= 10 ? `${item.stock_qty} in stock` : '✓ In stock'}
          </div>
        )}
