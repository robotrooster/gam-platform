...
/**
 * S629 (Nic): a signing link works WITHOUT a session.
 *
 * The path param is either a document id (signing from inside the portal, where
 * the session identifies the signer) or the signer's own 64-hex token from the
 * email, which identifies them by itself. Sending a stale or absent
 * Authorization header alongside a token is what turned an emailed link into a
 * login screen, so the header is omitted entirely when a token is in play.
 */
const SIGNER_TOKEN_RE = /^[a-f0-9]{64}$/i
export const isSignerToken = (v: string | undefined) => !!v && SIGNER_TOKEN_RE.test(v)
function authFetch(path: string, opts: RequestInit = {}) {
  const byToken = isSignerToken(path.split('/').pop()?.split('?')[0])
  const t = tok()
