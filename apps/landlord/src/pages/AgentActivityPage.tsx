...
export function AgentActivityPage() {
  ...
  const { data: summary, isLoading } = useQuery<SummaryData>(
    ['agent-activity-summary', days],
    () => apiGet<SummaryData>(`/landlord/agent-activity?days=${days}`),
  )

  const { data: recent = [] } = useQuery<RecentRow[]>(
    ['agent-activity-recent', outcomeFilter],
    () => apiGet<RecentRow[]>(
      `/landlord/agent-activity/recent?limit=50${outcomeFilter ? `&outcome=${outcomeFilter}` : ''}`),
  )
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <p style={{ fontSize: '.85rem', color: 'var(--text-2)', maxWidth: 720 }}>
            Conversations between your tenants / yourself and the GAM
            customer-service agents (Ava, Samantha, David, Sonny). This
            view shows metadata only — outcomes, tools used, escalations.
            Verbatim conversation text is admin-only.
          </p>
        </div>
