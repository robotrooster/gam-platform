...
export function DepotsPage() {
  ...
  const [editForm, setEditForm] = useState({
    name: '', street1: '', street2: '', city: '', state: '', zip: '', lat: '', lon: '',
  })
  ...
  const reload = async () => {
    try {
      const data = await apiGet<DepotRow[]>('/depots')
      setRows(data)
    } catch (e: any) {
      setErr(e?.response?.data?.error || 'Failed to load depots')
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await apiPost('/depots', {
        ...form,
        lat: Number(form.lat),
        lon: Number(form.lon),
      })
      setForm({ name: '', street1: '', street2: '', city: '', state: '', zip: '', lat: '', lon: '' })
      await reload()
    } catch (e: any) {
  ...
  const startEdit = (r: DepotRow) => {
    setEditing(r)
    setEditForm({
      name: r.name, street1: r.street1, street2: r.street2 ?? '',
  ...
  const onEditSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await apiPatch(`/depots/${editing.id}`, {
        name: editForm.name,
  ...
  return (
    <div>
      ...
      <div style={{ color: 'var(--text-2)', fontSize: 14, marginBottom: 24 }}>
        Where your trucks start and end each day. Coordinates are required
        for route generation — look up your yard's address on Google Maps
        and copy the lat/lon shown in the URL.
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 24 }}>
        <div>
          {loading ? (
            ...
            <div style={emptyStyle}>No depots yet. Add one to get started.</div>
          ) : (
        ...
        <div>
          <h2 style={h2Style}>Add a depot</h2>
          <form onSubmit={onSubmit} style={formStyle}>
            <label style={labelStyle}>Name</label>
            <input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })}
              required style={inputStyle} placeholder="Main Yard" />
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
              <div>
                ...
                <input value={form.lat} onChange={e => setForm({ ...form, lat: e.target.value })}
                  required type="number" step="any" style={inputStyle} placeholder="33.4484" />
              </div>
              <div>
                <label style={labelStyle}>Longitude</label>
                <input value={form.lon} onChange={e => setForm({ ...form, lon: e.target.value })}
                  required type="number" step="any" style={inputStyle} placeholder="-112.0740" />
              </div>
            </div>
        ...
        <Modal title={`Edit ${editing.name}`}
          ...
          footer={
            <>
              ...
              <button type="submit" form="depot-edit" disabled={editSaving}
                style={{ ...saveBtnStyle, opacity: editSaving ? 0.6 : 1 }}>
                {editSaving ? 'Saving…' : 'Save changes'}
              </button>
            </>
          }>
          <form id="depot-edit" onSubmit={onEditSubmit}>
            <label style={labelStyle}>Name</label>
