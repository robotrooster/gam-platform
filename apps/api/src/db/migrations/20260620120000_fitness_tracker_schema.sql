...
--
-- WHY: The fitness tracker backend (apps/api/src/routes/fitness.ts, mounted at
-- /api/fitness) ties into the existing tenant/landlord/admin JWT — any GAM user
-- can log workouts. Its 9 tables were originally created by hand from a loose
-- gold-fitness-migration.sql at the repo root, so they existed in dev but were
-- NOT tracked by the migration runner: a fresh DB or another machine would boot
-- without them. This re-homes that DDL into the runner as the single source of
-- truth. Every statement is IF NOT EXISTS, so this is a no-op against the dev DB
-- that already has the tables and a clean create everywhere else.
--
-- No backfill needed (new feature, no existing data depends on it beyond the
-- already-present dev rows).
