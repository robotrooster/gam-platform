...
export async function runAgentWithTools(input: RunWithToolsInput): Promise<RunWithToolsResult> {
   ...
   * guard forces.
   *
   * It was written as once per TURN, and that quietly disarmed every guard
   * under two-pass. The guards all push a correction and `continue`, so the
   * corrected draft is generated by the DECIDING pass — 1 KB of prompt, no
   * voice, no knowledge base. With a turn-scoped latch that draft was then
   * returned as-is, so the reply a person actually received on any corrected
   * turn was the terse internal draft, never the composed one.
   *
   * Caught on a landlord asking "tell me more about the apt 204 one". The
   * repetition guard fired correctly, handed back the get_unit_lease rows and
   * asked for a rewrite — and the rewrite, uncomposed, came back byte-identical
   * to the previous turn. The guard did its job; the composing pass it depended
   * on had been skipped.
   *
