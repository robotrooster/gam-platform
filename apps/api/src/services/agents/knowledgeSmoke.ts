 ...
 *
 * Indexes a handful of chunks (tenant / landlord), then runs a
...
const CHUNKS = [
  ...
  { scope: 'tenant' as const, title: 'Resetting your password', content: 'Use the Forgot Password link on the sign-in page. A reset link is emailed to the address on your account.' },
]

async function main() {
  console.log('[ksmoke] indexing 4 chunks (tenant/landlord)...')
  ...
  const hits = await retrieve(['tenant'], q, 3)
  for (const h of hits) {
