...
Once your bank is connected and verified, you can pay rent by bank (ACH). Payment is something you initiate each cycle, not an automatic pull — either from the Payments section, or by asking the assistant, which will tell you the total including any processing fee before it takes anything. If you'd rather it happened on its own, that's autopay, and the assistant can set that up too. If you're not sure whether your bank is set up yet, the support team can check your account and see whether a bank is connected and verified — and walk you to the right spot if it isn't.

A couple of things worth knowing: a bank (ACH) payment takes a little longer to clear than a card, and your bank needs enough funds when it's pulled, or the payment can be returned. If a payment is ever returned, that's handled through the banking system — the charge stays open — make sure the account is funded, then submit the payment again. For certain return reasons (like a disputed authorization), GAM pauses bank payments until your bank is re-verified; you can still pay by card in the meantime.
