  ...
  if (biz?.stripe_connect_account_id && biz.connect_payouts_enabled) {
    try {
      ...
      const session = await createInvoiceCheckoutSession({
        ...
        applicationFeeCents: Math.round((Math.round(Number(inv.total_amount) * 100)) * BIZ_FEES.BUSINESS_INVOICE_APP_FEE_PCT) + BIZ_FEES.BUSINESS_INVOICE_APP_FEE_FIXED_CENTS,
        businessConnectAccountId: biz.stripe_connect_account_id,
        invoiceNumber:            inv.invoice_number,
