...
import { getTerminal, discoverReaders, connectReader, collectCardPayment, cancelCurrentPayment } from '../lib/terminal'
...
export function POSPage() {
  ...
  const chargeWithReader = async () => {
    ...
    try {
      const res = await fetch((import.meta as any).env?.VITE_API_URL + '/api/terminal/create-payment-intent', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: 'Bearer ' + localStorage.getItem('gam_token') },
  ...
  return (
    <div>
          ...
          <div className="card" style={{position:'sticky',top:80}}>
            ...
            <div style={{marginBottom:10}}>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:5}}>
                {connectedReader && method==='card' && (
                <div style={{fontSize:'.72rem',color:'var(--green)',marginBottom:6,display:'flex',alignItems:'center',gap:4}}>
                  <span>●</span> {connectedReader.label || connectedReader.serial_number}
                  <button onClick={()=>{setConnectedReader(null)}} style={{background:'none',border:'none',color:'var(--text-3)',cursor:'pointer',fontSize:'.7rem',marginLeft:4}}>disconnect</button>
                </div>
              )}
            ...
            <button className="btn btn-primary" style={{width:'100%'}}
              ...
              onClick={()=> method==='card' ? chargeWithReader() : checkoutMut.mutate()}>
      ...
      {/* Reader Modal */}
      {readerModal && (
        <div className="modal-overlay" onClick={()=>setReaderModal(false)}>
          <div className="modal" style={{maxWidth:420}} onClick={e=>e.stopPropagation()}>
            <div className="modal-header">
              <span className="modal-title">Connect Card Reader</span>
              <button className="btn btn-ghost btn-sm" onClick={()=>setReaderModal(false)}>✕</button>
            </div>
            <div style={{padding:'0 24px 24px'}}>
              {terminalStatus==='discovering' && <div style={{textAlign:'center',padding:24,color:'var(--text-3)'}}>Searching for readers…</div>}
              {terminalStatus==='connecting' && <div style={{textAlign:'center',padding:24,color:'var(--text-3)'}}>Connecting…</div>}
              {terminalStatus==='error' && <div style={{color:'var(--red)',fontSize:'.82rem',marginBottom:12}}>{terminalError}</div>}
              {readers.length === 0 && terminalStatus==='idle' && (
                <div style={{textAlign:'center',padding:24}}>
                  <div style={{color:'var(--text-3)',fontSize:'.85rem',marginBottom:16}}>No readers found</div>
                  <button className="btn btn-primary" onClick={discoverAndConnect}>Search Again</button>
                </div>
              )}
              {readers.map((r:any) => (
                <div key={r.id} onClick={()=>connectToReader(r)}
                  style={{border:'1px solid var(--border-1)',borderRadius:8,padding:'12px 16px',marginBottom:8,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'space-between'}}
                  onMouseEnter={e=>(e.currentTarget.style.borderColor='var(--gold)')}
                  onMouseLeave={e=>(e.currentTarget.style.borderColor='var(--border-1)')}>
                  <div>
                    <div style={{fontWeight:600,fontSize:'.88rem'}}>{r.label || r.serial_number}</div>
                    <div style={{fontSize:'.75rem',color:'var(--text-3)'}}>{r.device_type} · {r.status}</div>
                  </div>
