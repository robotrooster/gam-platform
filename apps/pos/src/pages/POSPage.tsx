...
export function POSPage() {
  ...
  const createItemMut = useMutation(() => apiPost('/pos/items', { ...newItem, costPrice:Number(newItem.cost_price), sellPrice:Number(newItem.sell_price), taxRate:Number(newItem.tax_rate), chargeEligible:newItem.charge_eligible, stockQty:Number(newItem.stock_qty), stockMin:Number(newItem.stock_min), stockMax:Number(newItem.stock_max) }), { onSuccess: () => { qc.invalidateQueries('pos-items'); setNewItem({ name:'', category:'misc', icon:'📦', sell_price:'', cost_price:'', tax_rate:'0', charge_eligible:true, stock_qty:'0', stock_min:'5', stock_max:'50' }) } })
  ...
  const createTaxMut = useMutation(() => apiPost('/pos/tax-rates', { ...newTax, rate:Number(newTax.rate)/100, taxType:newTax.tax_type, appliesTo:newTax.applies_to==='all'?['all']:[newTax.applies_to] }), { onSuccess: () => { qc.invalidateQueries('pos-tax-rates'); setNewTax({ name:'', rate:'', tax_type:'state', applies_to:'all' }) } })
  ...
  const refundMut = useMutation(() => apiPost(`/pos/transactions/${refundModal.tx?.id}/refund`, { amount:Number(refundAmt)||refundModal.tx?.total, reason:refundReason, refundMethod:refundModal.tx?.payment_method }), { onSuccess: () => { qc.invalidateQueries('pos-transactions'); setRefundModal({show:false,tx:null}); setRefundAmt(''); setRefundReason('') } })
    ...
    <div>
          ...
          <div className="card" style={{position:'sticky',top:80}}>
            ...
            {method==='charge'&&(<div style={{marginBottom:10}}>
              <select className="form-select" value={tenantId} onChange={e=>setTenantId(e.target.value)} style={{width:'100%'}}>
                ...
                {(tenants as any[]).map((t:any)=><option key={t.id} value={t.id}>{t.first_name} {t.last_name}</option>)}
              </select>
        ...
        <div style={{display:'grid',gap:16}}>
          <div className="card">
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:12,marginTop:12}}>
              ...
              <div style={{display:'flex',alignItems:'center',gap:8,paddingTop:20}}><input type="checkbox" id="ce" checked={newItem.charge_eligible} onChange={e=>setNewItem(s=>({...s,charge_eligible:e.target.checked}))} /><label htmlFor="ce" style={{fontSize:'.82rem'}}>Charge eligible</label></div>
            </div>
            ...
            <table className="data-table">
              ...
              <tbody>
                {(items as any[]).map((item:any)=>(<tr key={item.id}>
                  ...
                  <td className="mono">{item.stock_qty>=999?'inf':item.stock_qty}</td>
                  <td><button onClick={()=>toggleChargeMut.mutate({id:item.id,val:!item.charge_eligible})} style={{background:item.charge_eligible?'var(--gold-bg)':'var(--bg-3)',border:"1px solid "+(item.charge_eligible?'var(--gold)':'var(--border-1)'),borderRadius:4,padding:'2px 8px',cursor:'pointer',fontSize:'.75rem',color:item.charge_eligible?'var(--gold)':'var(--text-3)'}}>{item.charge_eligible?'Yes':'No'}</button></td>
                  <td><button onClick={()=>toggleActiveMut.mutate({id:item.id,val:!item.is_active})} style={{background:'var(--bg-2)',border:'1px solid var(--border-1)',borderRadius:4,padding:'2px 8px',cursor:'pointer',fontSize:'.75rem',color:item.is_active?'var(--green)':'var(--text-3)'}}>{item.is_active?'Active':'Off'}</button></td>
                  <td><button className="btn btn-ghost btn-sm" onClick={()=>setEditItem({...item,_sell:String(item.sell_price),_cost:String(item.cost_price),_tax:String(Number(item.tax_rate)*100),_stock:String(item.stock_qty),_min:String(item.stock_min),_max:String(item.stock_max)})}>Edit</button></td>
                </tr>))}
              </tbody>
            </table>
          </div>
        ...
        <div style={{display:'grid',gap:16}}>
          <div className="card">
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:12,marginTop:12}}>
              ...
              <div><div style={{fontSize:'.75rem',color:'var(--text-3)',marginBottom:4}}>Type</div><select className="form-select" value={newTax.tax_type} onChange={e=>setNewTax(s=>({...s,tax_type:e.target.value}))} style={{width:'100%'}}>{TAX_TYPES.map(t=><option key={t} value={t}>{t}</option>)}</select></div>
              <div><div style={{fontSize:'.75rem',color:'var(--text-3)',marginBottom:4}}>Applies To</div><select className="form-select" value={newTax.applies_to} onChange={e=>setNewTax(s=>({...s,applies_to:e.target.value}))} style={{width:'100%'}}><option value="all">All categories</option>{CATEGORIES.map(c=><option key={c} value={c}>{c}</option>)}</select></div>
            </div>
            ...
            <table className="data-table">
              ...
              <tbody>
                {(taxRates as any[]).length?(taxRates as any[]).map((r:any)=>(<tr key={r.id}>
                  <td style={{fontWeight:500}}>{r.name}</td><td><span className="badge badge-muted">{r.tax_type}</span></td>
            ...
            <table className="data-table">
              ...
              <tbody>
                {(vendors as any[]).length?(vendors as any[]).map((v:any)=>(<tr key={v.id}>
                  ...
                  <td className="mono">{v.lead_time_days}d</td>
            ...
            <div style={{marginTop:16,borderTop:'1px solid var(--border-1)',paddingTop:16}}>
              ...
              <div style={{display:'grid',gridTemplateColumns:'2fr 1fr 1fr auto',gap:8,alignItems:'end',marginBottom:8}}>
                <div><div style={{fontSize:'.72rem',color:'var(--text-3)',marginBottom:3}}>Item</div>
                  <select className="form-select" style={{width:'100%'}} value={poItemRow.itemId} onChange={e=>{const it=(items as any[]).find((x:any)=>x.id===e.target.value);setPoItemRow(s=>({...s,itemId:e.target.value,unitCost:it?String(it.cost_price):s.unitCost}))}}>
              ...
              <tbody>
                {(purchaseOrders as any[]).length?(purchaseOrders as any[]).map((po:any)=>(<>
                  <tr key={po.id} style={{cursor:'pointer'}} onClick={()=>setExpandedPO(expandedPO===po.id?null:po.id)}>
                    <td className="mono" style={{color:'var(--gold)',fontWeight:600}}>{po.po_number}</td>
                    <td style={{fontWeight:500}}>{po.vendor_name}</td>
                    <td className="mono">{po.item_count}</td>
                    ...
                    <td style={{fontSize:'.82rem',color:'var(--text-3)'}}>{po.expected_date?new Date(po.expected_date).toLocaleDateString():'—'}</td>
                  ...
                  {expandedPO===po.id&&po.items&&(<tr key={po.id+'-exp'}>
                    <td colSpan={7} style={{background:'var(--bg-2)',padding:'8px 16px'}}>
                      <table style={{width:'100%',fontSize:'.8rem'}}>
                        ...
                        <tbody>{po.items.map((li:any)=>(<tr key={li.id}><td style={{padding:'2px 8px'}}>{li.item_name}</td><td className="mono" style={{textAlign:'right',padding:'2px 8px'}}>{li.qty_ordered}</td><td className="mono" style={{textAlign:'right',padding:'2px 8px'}}>{fmt(li.unit_cost)}</td><td className="mono" style={{textAlign:'right',padding:'2px 8px'}}>{fmt(li.subtotal)}</td></tr>))}</tbody>
                      </table>
                    </td>
                  </tr>)}
        ...
        <div style={{display:'grid',gap:16}}>
          {(lowStock as any[]).length>0&&(<div className="card" style={{borderColor:'var(--amber)'}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(200px,1fr))',gap:10,marginTop:12}}>
              {(lowStock as any[]).map((item:any)=>(<div key={item.id} style={{background:'var(--bg-1)',borderRadius:8,padding:'10px 14px',border:'1px solid var(--border-1)'}}>
                ...
                <div style={{fontSize:'.75rem',color:'var(--text-3)',marginTop:2}}>{item.vendor_name||'No vendor linked'}</div>
                <div style={{marginTop:6,display:'flex',justifyContent:'space-between'}}>
                  <span style={{color:'var(--amber)',fontWeight:700}}>{item.stock_qty} left</span>
                  <span style={{fontSize:'.72rem',color:'var(--text-3)'}}>min {item.stock_min}</span>
                </div>
              </div>))}
            </div>
          </div>)}
          <div className="card" style={{padding:0}}>
            ...
            <table className="data-table">
              ...
              <tbody>
                {(items as any[]).filter((i:any)=>i.stock_qty<999).map((item:any)=>(<tr key={item.id}>
                  <td style={{fontWeight:500}}>{item.icon} {item.name}</td>
                  <td><span className="badge badge-muted">{item.category}</span></td>
                  <td className="mono" style={{fontWeight:700,color:item.stock_qty===0?'var(--red)':item.stock_qty<=item.stock_min?'var(--amber)':'var(--text-0)'}}>{item.stock_qty}</td>
                  <td className="mono" style={{color:'var(--text-3)'}}>{item.stock_min}</td>
                  <td className="mono" style={{color:'var(--text-3)'}}>{item.stock_max}</td>
                  <td>{item.stock_qty===0?<span className="badge badge-red">Out</span>:item.stock_qty<=item.stock_min?<span className="badge badge-amber">Low</span>:<span className="badge badge-green">OK</span>}</td>
            ...
            <table className="data-table">
              ...
              <tbody>
                {(inventoryLog as any[]).length?(inventoryLog as any[]).map((log:any)=>(<tr key={log.id}>
                  <td className="mono" style={{fontSize:'.78rem',color:'var(--text-3)'}}>{new Date(log.created_at).toLocaleDateString()}</td>
                  <td style={{fontWeight:500}}>{log.item_icon} {log.item_name}</td>
                  <td className="mono" style={{fontWeight:700,color:log.change_qty>0?'var(--green)':'var(--red)'}}>{log.change_qty>0?'+':''}{log.change_qty}</td>
                  <td className="mono">{log.stock_before}</td>
                  <td className="mono">{log.stock_after}</td>
      ...
      {editVendor&&(<div className="modal-overlay" onClick={()=>setEditVendor(null)}><div className="modal" style={{maxWidth:520}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{padding:'0 24px 24px',display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
          ...
          <div><div style={{fontSize:'.75rem',color:'var(--text-3)',marginBottom:4}}>Contact</div><input className="form-input" style={{width:'100%'}} value={editVendor.contact_name||''} onChange={e=>setEditVendor((s:any)=>({...s,contact_name:e.target.value}))} /></div>
          ...
          <div><div style={{fontSize:'.75rem',color:'var(--text-3)',marginBottom:4}}>Lead Time (days)</div><input className="form-input" type="number" style={{width:'100%'}} value={editVendor.lead_time_days||3} onChange={e=>setEditVendor((s:any)=>({...s,lead_time_days:Number(e.target.value)}))} /></div>
          <div style={{display:'flex',alignItems:'center',gap:8,paddingTop:20}}><input type="checkbox" id="va" checked={editVendor.is_active} onChange={e=>setEditVendor((s:any)=>({...s,is_active:e.target.checked}))} /><label htmlFor="va" style={{fontSize:'.82rem'}}>Active</label></div>
          <div style={{gridColumn:'1/-1',marginTop:8}}><button className="btn btn-primary" style={{width:'100%'}} onClick={()=>updateVendorMut.mutate({name:editVendor.name,contactName:editVendor.contact_name,email:editVendor.email,phone:editVendor.phone,leadTimeDays:editVendor.lead_time_days,isActive:editVendor.is_active})} disabled={updateVendorMut.isLoading}>{updateVendorMut.isLoading?'Saving...':'Save Changes'}</button></div>
        </div>
      </div></div>)}
      ...
      {readerModal&&(<div className="modal-overlay" onClick={()=>setReaderModal(false)}><div className="modal" style={{maxWidth:420}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{padding:'0 24px 24px'}}>
          ...
          {readers.map((r:any)=>(<div key={r.id} onClick={()=>connectToReader(r)} style={{border:'1px solid var(--border-1)',borderRadius:8,padding:'12px 16px',marginBottom:8,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'space-between'}} onMouseEnter={e=>(e.currentTarget.style.borderColor='var(--gold)')} onMouseLeave={e=>(e.currentTarget.style.borderColor='var(--border-1)')}>
            <div><div style={{fontWeight:600,fontSize:'.88rem'}}>{r.label||r.serial_number}</div><div style={{fontSize:'.75rem',color:'var(--text-3)'}}>{r.device_type} - {r.status}</div></div>
